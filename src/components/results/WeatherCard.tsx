...
const statusIcons: Record<string, React.JSX.Element> = {
  ...
  Warm: <Sun size={56} className="text-yellow-200" />, // softer sun icon
  Mild: <Sun size={56} className="text-green-200" />, // gentle sun for mild weather
...
const statusBackgrounds: Record<string, string> = {
  ...
  Warm: "from-yellow-200/90 via-yellow-300/80 to-orange-300/90", // lighter, warm gradient
  Mild: "from-green-200/90 via-green-300/80 to-emerald-300/90", // gentle, mild gradient
...
export default function WeatherCard({
  ...
  return (
    ...
    >
      ...
      {/* Status */}
      <p className="text-3xl font-extrabold z-10 mb-2 drop-shadow-lg">
        {status}
      </p>
      ...
      {category && (
        <div className="z-10 text-center mb-3">
          <p className="text-lg font-semibold">{category.quantity}</p>
          <p className="text-sm opacity-85 leading-snug max-w-[200px] mx-auto">
